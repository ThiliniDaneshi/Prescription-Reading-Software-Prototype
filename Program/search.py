...
while(searchnext=='true'):
    with open("MedicinesDb04.csv", "r") as data:
        ...
        if(len(list00)>1): 
            ...
            if(checkback !=""):
                for inlist0 in list0:
                    ...
                    l=len(name)
					#set(str1) <= set(str2)	 or (set(checkback.casefold()) <= set(name[l-1].casefold()))) 
					#if((checkback.casefold()) <= (name[(l-3):l].casefold()))
                    if( set(checkback.casefold()) < set(name[(l-4):].casefold())): # by last letter
        ...
        if(len(list3)>1):
            ...
            for inlist3 in list3:
                rowfreq=list3[inlist3][6]
                rowform=list3[inlist3][5]
                rowroute=list3[inlist3][4]
